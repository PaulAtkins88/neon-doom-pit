...
import { createEntityId, type EntityId, type PlayerSnapshot, type WeaponType } from '@neon/shared';
...
export class Player {
  readonly id: EntityId = createEntityId();
  ...
  updatePresentation(deltaSeconds: number, keyState: Record<string, boolean>): void {
    const moving = keyState.KeyW || keyState.KeyA || keyState.KeyS || keyState.KeyD;
    const sway = Math.sin(performance.now() * 0.012) * (moving ? 0.02 : 0.008);
    this.gun.position.y = -0.36 + sway;
    this.gun.rotation.z = THREE.MathUtils.lerp(this.gun.rotation.z, 0, 0.16);
  ...
  applyAuthoritativeSnapshot(snapshot: PlayerSnapshot, applyRotation = true): void {
    ...
    this.shotgunCharges = snapshot.shotgunCharges;
    this.kills = snapshot.kills;
    this.reloadTimer = snapshot.reloadTimer;
    this.fireCooldown = snapshot.fireCooldown;
  }
