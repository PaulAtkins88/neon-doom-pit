...
import { CAMERA_START_POSITION, PICKUP_RADIUS, PICKUP_RESPAWN_DELAY, PICKUP_SPAWN_POINTS, PLAYER_PROJECTILE_DAMAGE, PLAYER_PROJECTILE_HIT_PADDING, PLAYER_PROJECTILE_RADIUS, PLAYER_PROJECTILE_SPEED, PLAYER_PROJECTILE_TTL, PLAYER_SHOTGUN_PELLETS, SHOTGUN_PICKUP_CHARGES } from '../config/gameConfig';
...
export class Game {
  ...
  private updatePlayer(deltaSeconds: number): void {
    ...
    if (this.player.consumeReloadCompletion()) {
      ...
      this.hud.updateHud(this.player);
    }
  }
  ...
  private startRun(): void {
    this.hud.hideOverlay();
    if (this.state.gameOver || this.monsters.length === 0) {
      this.resetGame();
    }
    this.state.active = true;
  ...
  private collectPickup(pickup: Pickup): void {
    if (pickup.type === 'health') {
      ...
      this.hud.showPickup(`Shotgun acquired: ${PLAYER_SHOTGUN_PELLETS} pellets x ${SHOTGUN_PICKUP_CHARGES} shots.`);
    }
